export const IDEA_TYPES = Object.freeze({
  hook: 'Hook',
  concept: 'Concept',
  phrase: 'Phrase',
  melody: 'Melody',
...
export const SONG_SECTIONS = Object.freeze(['intro','verse','pre_chorus','chorus','post_chorus','bridge','outro']);

const id = () => crypto.randomUUID();
const now = () => new Date().toISOString();
const clean = value => String(value ?? '').trim();

export function normalizeIdea(input={}) {
...
export function createIdea(input) { return normalizeIdea(input); }
export function createSong(input) { return normalizeSong(input); }
export function createVersion({ songId, body='', parentVersionId=null, number=1, changeNote='' }) {
...
export function matchesIdea(idea, query) {
...
export function nextVersionNumber(versions, songId) {
