...
export const supabaseConfigured = !placeholder(url) && !placeholder(publishableKey) && /^https:\/\/[^\s/]+\.supabase\.co$/i.test(url);
export const supabase = supabaseConfigured ? { auth: { configured: true } } : null;
...
export async function uploadAudio(userId, ideaId, blob, kind='voice_note'){
...
export async function requestAI(task, context){
  const token = await accessToken();
