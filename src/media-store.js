...
export async function saveAudio({ideaId,blob,mimeType,durationMs}){const db=await openDB();const record={id:crypto.randomUUID(),ideaId,blob,mimeType,durationMs:durationMs||null,createdAt:new Date().toISOString()};return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readwrite');tx.objectStore(STORE).put(record);tx.oncomplete=()=>resolve(record);tx.onerror=()=>reject(tx.error)})}
export async function getAudioForIdea(ideaId){const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readonly');const req=tx.objectStore(STORE).getAll();req.onsuccess=()=>resolve(req.result.filter(x=>x.ideaId===ideaId));req.onerror=()=>reject(req.error)})}
